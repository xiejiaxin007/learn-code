...
<script type="text/babel">
  ...
  export default {
    ...
    methods: {
      ...
      syncPostion() {
        // kangx：节流时间
        let throttleTime = this.scrollDom ? 50 : 20;
        return throttle(throttleTime, () => {
          ...
          if (this.scrollDom) {
            ...
            // 结束索引，截取多1位可以让高度滑动，layout代码就不用动
            ...
            this.paddingBottom = allHeight - this.itemHeight * this.showNum - scrollTop;
          }
        });
      },
      ...
      doLayout() {
        // kangx：resize后重新计算itemHeight高度，itemHeight变化会引起 shouldUpdateHeight变化，重新计算顶部高度等一系列数据，所以此处要放在上面
        if (this.scrollDom) {
          const td = this.bodyWrapper.querySelector('.el-table__body tr td');
          if (td) {
            const height = td.offsetHeight;
            this.itemHeight = height || 48;
          }
        }
    ...
    computed: {
      ...
      bodyHeight() {
        ...
        } else if (this.itemHeight && this.scrollDom) {
          // 是虚拟DOM且定义了单元格高度
          // 横向滑动条高度
          let gutterHeight = this.layout.scrollX ? this.layout.gutterWidth : 0;
          return {
            // 此处减一是为了table-layout的第50行，让高度有个差值，可以滚动起来
            'max-height': (this.itemHeight * this.showNum - 1 + gutterHeight) + 'px'
          };
        }
        return {};
      },
    ...
    watch: {
      ...
      // kangx：设定最大高度，在这赋值是因为resize时itemHeight变化了
      allHeight: {
        immediate: true,
        handler() {
