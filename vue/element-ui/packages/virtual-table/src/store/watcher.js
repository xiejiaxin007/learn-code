...
export default Vue.extend({
  ...
  methods: {
    ...
    cleanSelection() {
      const states = this.states;
      const { data, rowKey, selection, scrollDom, allData } = states;
      let deleted;
      if (rowKey) {
        deleted = [];
        ...
        // kangx：虚拟DOM就把所有数据选中
        let dataList = scrollDom ? allData : data;
        deleted = selection.filter(item => dataList.indexOf(item) === -1);
      }
      if (deleted.length) {
    ...
    _toggleAllSelection() {
      const states = this.states;
      const { data = [], selection, scrollDom, allData = [] } = states;
    ...
    updateAllSelected() {
      const states = this.states;
      const { selection, rowKey, selectable, scrollDom, data = [], allData = [] } = states;
